  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      if(user){
        ...
        console.log(user);
        return res.status(400).json({success,errors:"Soory a user already exists"})
      }
      ...
      // .then(user => res.json(user)).catch(err=>console.log("Email repeated",err));
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try{
        ...
        const passcomp=await bcrypt.compareSync(req.body.password,user.password);
        if(passcomp){
          var data={
            user:{
              id:user.id
